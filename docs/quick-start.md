...
Процессы - это коллекции сгруппированных по назначению действий и статусов. 

Они созданы для использования совместно с админ-панелью, однако их необходимо добавлять и в те приложения, 
что не используют админ-панель, иначе вы получите ошибки.
...
```
Обратите внимание на символьное имя процесса (`name: 'order'`), указанное в атрибуте `#[AsWorkflow]`.
Такое же имя процесса вы будете указывать затем у сущностей, действий и статусов в их атрибутах: 
 `#[AsEntity]`, `#[AsAction]`и `#[AsStatus]` соответственно.
...
В принципе, этого достаточно для формирования корректной миграции, которая создаст таблицу с логом действий над заказами.
...
```
_Возможно, что в зависимости от версии Symfony, вам потребуются дополнительные шаги для настройки маппинга кастомного типа.
Следуйте руководству по Symfony и Doctrine._ 
...
Примечания:
> - В реальном действии в методе `can()` мы бы написали предусловия - тои есть условия, когда действие может быть выполнено.
...
Применение действия можно записать чуть проще, учитывая тот факт, что в метод `__invoke()` можно сразу передать сущность и контекст:
```php
...
        try {
            ($this->orderPayAction)(
